...
| `integrations/claude_code/*.py` | no | the two hook scripts themselves — plain stdlib, always exit 0 |
...
| `app_id` / `app_key` at runtime | `RunApp.run` passes them into `bind_sleep_wake` and on into peripheral auth, not only into `deploy`. `main.py` reads them from `RAVEN_APP_ID` / `RAVEN_APP_KEY`. Whether RavenOS puts them there for a deployed app is not visible in the public source and is untested. If gaze or the button misbehave on a real device, this is the first thing to check. Our `EyeControl()` call also does not thread credentials through — on device that may need fixing once the mechanism is known. |
